...
export async function sendEmail(content) {
    ...
    const recipient = content.error ? emailConfigs[content.emailConfig].error : emailConfigs[content.emailConfig].to
    const bcc = content.error ? undefined : emailConfigs[content.emailConfig].bcc

    // send mail with defined transport object
    return await transporter.sendMail({
        from: `"${emailConfigs[content.emailConfig].name}" <${emailConfigs[content.emailConfig].user}>`,   // sender address
        to: recipient,                          // list of receivers, comma separated
        bcc: bcc,                               // list of bcc receivers, comma separated
